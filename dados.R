...
# Banco principal
ENEM_2023 <- read_delim(file = "MICRODADOS_ENEM_2023.csv", delim = ";", locale = locale(decimal_mark = ".", encoding = "ISO-8859-1"))
...
ENEM_2023$IN_TREINEIRO <- factor(ENEM_2023$IN_TREINEIRO, levels = c(1,0),  labels=c('Sim','Não')) # Check

ENEM_2023$TP_LOCALIZACAO_ESC <- factor(ENEM_2023$TP_LOCALIZACAO_ESC, levels = c(1,2), labels=c('Urbana','Rural')) # Interessante

ENEM_2023$TP_SEXO <- factor(ENEM_2023$TP_SEXO, levels = c('M','F'), labels=c('Masculino','Feminino')) # Machismo

ENEM_2023$TP_PRESENCA_CN <- factor(ENEM_2023$TP_PRESENCA_CN, levels = c(0,1,2), # Check
                                   labels=c('Faltou à prova',
                                            'Presente na prova',
                                            'Eliminado na prova'))

ENEM_2023$TP_PRESENCA_CH <- factor(ENEM_2023$TP_PRESENCA_CH, levels = c(0,1,2), # Check
                                   labels=c('Faltou à prova',
                                            'Presente na prova',
                                            'Eliminado na prova'))

ENEM_2023$TP_PRESENCA_LC <- factor(ENEM_2023$TP_PRESENCA_LC, levels = c(0,1,2), # Check
                                   labels=c('Faltou à prova',
                                            'Presente na prova',
                                            'Eliminado na prova'))

ENEM_2023$TP_PRESENCA_MT <- factor(ENEM_2023$TP_PRESENCA_MT, levels = c(0,1,2), # Check
                                   labels=c('Faltou à prova',
                                            'Presente na prova',
                                            'Eliminado na prova'))

ENEM_2023$TP_STATUS_REDACAO <- factor(ENEM_2023$TP_STATUS_REDACAO, levels = c(1,2,3,4,5,6,7,8,9), # Check
...
ENEM_2023$Q006 <- factor(ENEM_2023$Q006,levels =  c('A','B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','Q'), # Renda Familiar
                         labels=c('Nenhuma Renda','Até R$ 1.320,00','De R$ 1.320,01 até R$ 1.980,00.',
                                  ...
                                  'De R$ 7.920,01 até R$ 9240,00.','De R$ 9.240,01 até R$ 10.560,00.',
...
ENEM_2023$CLASSE_SOCIAL <- case_when(
  ENEM_2023$Q006 == 'Nenhuma Renda' ~ 'Não informado',
  ENEM_2023$Q006 %in% c('Até R$ 1.320,00', 'De R$ 1.320,01 até R$ 1.980,00.',
                        'De R$ 1.980,01 até R$ 2.640,00.') ~ "Baixa",
  ENEM_2023$Q006 %in% c('De R$ 2.640,01 até R$ 3.300,00.', 'De R$ 3.300,01 até R$ 3.960,00.', 
                        'De R$ 3.960,01 até R$ 5.280,00.', 'De R$ 5.280,01 até R$ 6.600,00.') ~ "Média",
  ENEM_2023$Q006 %in% c('De R$ 6.600,01 até R$ 7.920,00.', 'De R$ 7.920,01 até R$ 9.240,00.', 
                        ...
                        'Acima de R$ 26.400,00.') ~ "Alta"
)

# Filtragem
ENEM_2023_LIMPO <- ENEM_2023 %>% 
  select(TP_FAIXA_ETARIA, IN_TREINEIRO, TP_LOCALIZACAO_ESC, TP_SEXO, TP_PRESENCA_CN, TP_PRESENCA_CH, TP_PRESENCA_LC,
         TP_PRESENCA_MT, TP_STATUS_REDACAO, NO_MUNICIPIO_ESC, CO_UF_ESC, CLASSE_SOCIAL, NU_NOTA_CH, NU_NOTA_CN,
  ...
  filter(
    ...
    CO_UF_ESC == 35                     &
    CLASSE_SOCIAL != "Não informado"
    ) %>% 
    drop_na()

ENEM_2023_LIMPO <- ENEM_2023_LIMPO %>% 
  select (-c(IN_TREINEIRO, TP_PRESENCA_CN, TP_PRESENCA_CH, TP_PRESENCA_LC,
          TP_PRESENCA_MT, TP_STATUS_REDACAO, CO_UF_ESC))

colnames(ENEM_2023_LIMPO) <- c("Faixa Etária", "Área Escolar", "Sexo", "Município", "Classe Social", 
                               "Nota CH", "Nota CN", "Nota LC", "Nota MT", "Nota RD")

#write.table(ENEM_2023_LIMPO, "ENEM_2023_LIMPO.csv", sep = ";", dec = ".", row.names = FALSE)
